...
if [ $KSU_ENABLE -eq 1 ]; then
    echo "KSU is enabled"
#    yes | unzip susfs.zip
    wget -O setup.sh https://raw.githubusercontent.com/mmxdxmm/KernelSU-Next/next-susfs/kernel/setup.sh
...
make CFLAGS="$CFLAGS" CXXFLAGS="$CFLAGS" $MAKE_ARGS -j$(nproc) ${TARGET_DEVICE}_defconfig
